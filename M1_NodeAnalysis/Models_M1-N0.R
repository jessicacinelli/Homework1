...
interarrivals <- read.table("ffdatools/tuples-R63-M1-N0-240/interarrivals.txt")
...
hex_mod<-nls (r ~ 0.5*exp(-(l1*t))+0.5*exp(-(l2*t)), start=list(l1=(1/mean(interarrivals$V1)),l2=1.486078e-06 ))

#lines(t, predict(hex_mod), col="magenta", lwd=4)
ks.test(r, predict(hex_mod))

# Stima delle regressione: modello iperesponenzionale (2)
hex2_mod<-nls (r ~ 0.2*exp(-(l1*t))+0.8*exp(-(l2*t)), start=list(l1=(1/mean(interarrivals$V1)),l2=1.486078e-06 ))
