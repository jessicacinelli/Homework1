...
#calcolo dei frequent itemset
freq_oct<-apriori(data_oct, parameter =list(support=s_oct, confidence=0.75, minlen=3, maxlen=20, target="frequent itemsets")) 

#calcolo delle regole associative
rules_oct<-apriori(data_oct, parameter =list(support=s_oct, confidence=0.80, minlen=3, maxlen=20, target="rules")) 

#inspect(sort(rules_oct, by="confidence"))
#mostra solo le prime 40 regole ordinate per confidenza
#inspect(head(sort(rules_oct, by = "confidence"), 40)) 
#inspect(freq_oct)
